function [o1, o2] = applyLinearCrossover(p1, p2,pC, FitnessValue,R,empty_individual)
...
if rand() < pC
    ...
    %calculate the cost
    offSprings(1).Cost = FitnessValue( offSprings(1).Position(1),  offSprings(1).Position(2),R);

    offSprings(2).Position= (3/2).*p1 - (1/2).*p2;
    %calculate the cost
    offSprings(2).Cost = FitnessValue( offSprings(2).Position(1),  offSprings(2).Position(2),R);

    offSprings(3).Position = (3/2).*p2 - (1/2).*p1;
    %calculate the cost
    offSprings(3).Cost = FitnessValue( offSprings(3).Position(1),  offSprings(3).Position(2),R);
