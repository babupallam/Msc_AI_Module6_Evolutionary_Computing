...
%since we use (mu, lamdba) survivor selection technique, we use only
%offspring solutions in order to get the result and making sure that it is
%even for simplyfiying the operations when crossover operator is applied
nC = mod(nPop, 2)+nPop;;
...
for it = 1:MaxIt
    ...
    for k = 1:nC/2
        ...
        [popc(k, 1).Position, popc(k, 2).Position] = applyLinearCrossover(popc(k,1).Position, popc(k,2).Position, pC, FitnessValue,R,empty_individual );
